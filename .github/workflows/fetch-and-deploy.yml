...
def job_sources(country: str) -> list[tuple[str, str]]:
    ...
    # Country-specific job board RSS / search feeds (where available)
    extra = {
        "Czech Republic": [
            ("jobs.cz",    "https://www.jobs.cz/rss/nabidky/?q={q}&locality.code="),
            ("prace.cz",   "https://www.prace.cz/rss/?phrase={q}"),
        ],
        "Poland": [
            ("pracuj.pl",  "https://www.pracuj.pl/praca/{q};kw/feed.rss"),
            ("olx.pl",     "https://www.olx.pl/praca/q-{q}/?view=list&search%5Border%5D=filter_float_price:desc"),
        ],
        "Hungary": [
            ("profession.hu", "https://www.profession.hu/rss/allasok?kulcsszo={q}"),
        ],
        "Romania": [
            ("ejobs.ro",   "https://www.ejobs.ro/rss/joburi/{q}/"),
            ("bestjobs.eu","https://www.bestjobs.eu/ro/rss/jobs?search_term={q}"),
        ],
        "Turkey": [
            ("kariyer.net","https://www.kariyer.net/rss/is-ilanlari?q={q}"),
        ],
        "Slovakia": [
        ...
        "United Kingdom": [
            ...
            ("totaljobs.com","https://www.totaljobs.com/jobs/{q}/rss"),
        ],
        "France": [
            ("apec.fr",    "https://www.apec.fr/rss/offres.rss?motsCles={q}"),
        ],
        "Belgium": [
            ("stepstone.be","https://www.stepstone.be/5/jobsearch-direct-entry.html?so=22&ac=bln&q={q}"),
        ],
        "Switzerland": [
        ...
        "Germany": [],   # Austria/Switzerland use German boards above
    }
...
def feed_queries(country: str):
    ...
    return [
        ...
        ("ma",          f"DEKRA SGS Eurofins {c} acquisition merger deal"),
        ("ma2",         f"Bureau Veritas Intertek {c} laboratory buyout 2025"),
        ("market",      f"{c} certification inspection regulation laboratory"),
        ("regulatory",  f"CSRD DORA AI Act certification compliance 2025"),
        ("tenders",     f"{c} inspection certification tender contract government"),
        ("standards",   f"{c} ISO standard accreditation laboratory 2025"),
...
def fetch_world_bank(wb_code: str) -> dict:
    ...
    for key, ind in [("gdp","NY.GDP.MKTP.KD.ZG"),("inflation","FP.CPI.TOTL.ZG")]:
        try:
            ...
            log.warning("  ✗ WorldBank %s %s %s", key, wb_code, exc)
            result[key] = None
    return result
