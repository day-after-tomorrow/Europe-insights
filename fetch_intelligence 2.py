...
CUTOFF_YEAR = 2026   # Only accept 2026+ articles
...
# ── TIC companies ─────────────────────────────────────────────────────────────
TIC_COMPANIES = [
    "DEKRA", "TÜV", "SGS", "Bureau Veritas", "Intertek", "Eurofins", "DNV",
]

# ── Hiring Signals: Job board RSS + Google News fallback ──────────────────────
# Two-layer strategy:
#   Layer 1 — Local job board RSS (direct postings, highest signal quality).
#             Each board is queried with URL-encoded company name.
#             Errors are caught silently; Google News fallback compensates.
#   Layer 2 — Google News RSS for hiring/expansion news about TIC companies.
#             Country relevance filter NOT applied here — the query already
#             contains the country name, so results are already anchored.
#             Applying the filter would drop valid results like:
#               "SGS opens new Czech lab" → "czech" IS in title → passes anyway
#               "Bureau Veritas expands Warsaw operations" → "warsaw" passes too
#             But it would also drop: "Intertek adds 50 engineers to Polish team"
#             ("polish" not in title but country IS Poland). So: no filter on jobs.

# Confirmed job board RSS endpoints (query-param format, no auth required).
# Each URL template contains {q} which is replaced with URL-encoded company name.
JOB_BOARDS = {
...
# Google News fallback queries for hiring signals.
# Country name is embedded in the query — NO secondary relevance filter applied.
# One query per company keeps the total manageable; results capped at 2 per query.
def _gnews_hiring_queries(country):
    # type: (str) -> List[str]
    c = sn(country)
    queries = [
        f"DEKRA {c} hiring jobs laboratory",
        f"SGS {c} recruitment jobs inspection",
        f"Bureau Veritas {c} jobs hiring",
        f"Intertek {c} hiring certification jobs",
        f"Eurofins {c} laboratory jobs hiring",
        f"TÜV {c} jobs certification hiring",
        f"DNV {c} hiring inspection jobs",
        f"{c} testing certification inspection laboratory jobs 2025",
    ]
    if country == "Austria":
        queries.insert(0, "TÜV Austria Stellen Wien jobs certification")
    return queries


def _fetch_board_rss(board_name, url_tpl, company, country):
    # type: (str, str, str, str) -> List[Dict]
    """Fetch one job board RSS feed for one company. Silent on failure."""
    url = url_tpl.replace("{q}", quote(company))
    headers = {
        "User-Agent": (
    ...
    try:
        ...
        for entry in feed.entries[:5]:
            ...
            # Keep items with no date (many boards omit pubDate per entry).
            # Only reject if date is explicitly present and clearly too old.
        ...
        if items:
            log.info("  ✓ JOB-BOARD %-14s (%s/%-14s) %d items",
                     board_name, country, company, len(items))
        return items
    except Exception as exc:
        log.debug("  ✗ JOB-BOARD %-14s (%s/%s): %s", board_name, country, company, exc)
        return []


def _fetch_gnews_hiring(query, country):
    # type: (str, str) -> List[Dict]
    """Fetch hiring signals from Google News RSS. No country relevance filter —
    the query already contains the country name."""
    try:
        resp   = requests.get(gnews_url_en(query), headers=HEADERS_NEWS, timeout=TIMEOUT)
        resp.raise_for_status()
        parsed = feedparser.parse(resp.content)
        items  = []
        for entry in parsed.entries[:2]:    # cap at 2 — board results take priority
...
def fetch_job_postings(country):
    # type: (str) -> List[Dict]
    """
    Fetch hiring signals using two layers:
      1. Local job board RSS (highest quality — direct postings)
      2. Google News hiring queries (fallback and supplement)
    Returns up to 10 deduplicated items sorted newest-first.
    """
    all_jobs = []
    futures  = []

    with ThreadPoolExecutor(max_workers=20) as pool:
        # Layer 1: job board RSS — query each board with each TIC company name
        board = JOB_BOARDS.get(country)
        if board:
            board_name, url_tpl = board
            for company in TIC_COMPANIES:
                futures.append(pool.submit(_fetch_board_rss,
                                           board_name, url_tpl, company, country))

        # Layer 2: Google News hiring queries
        for q in _gnews_hiring_queries(country):
            futures.append(pool.submit(_fetch_gnews_hiring, q, country))

        for fut in as_completed(futures):
            all_jobs.extend(fut.result())

    # Deduplicate on lowercased title prefix
    seen, unique = set(), []
    for job in all_jobs:
    ...
    # Sort: board postings first (local=True), then by date descending
    def sort_key(item):
        dt = parse_pub_date(item.get("pubDate", ""))
        ts = dt.timestamp() if dt else 0
        return (0 if item.get("local") else 1, -ts)
    unique.sort(key=sort_key)

    log.info("  ✓ HIRING (%s) %d signals (%d board, %d gnews)",
             country,
             len(unique[:10]),
             sum(1 for j in unique[:10] if j.get("local")),
             sum(1 for j in unique[:10] if not j.get("local")))
    return unique[:10]
...
# TÜV SÜD is us — articles about ourselves are not competitive intelligence.
# "TÜV" alone is NOT excluded because TÜV Rheinland, TÜV Nord, TÜV Austria are
# competitors. We only exclude explicit "TÜV SÜD" / "TÜV Süd" mentions.
TUVSUD_VARIANTS = [
    "tüv süd", "tuv sud", "tüvsüd", "tuvsud",
    "tüv sued",  # occasional transliteration
]

def is_tuvsud(title: str, source: str) -> bool:
    """Return True if the item is about TÜV SÜD (us) — should be excluded."""
    haystack = (title + " " + source).lower()
    return any(v in haystack for v in TUVSUD_VARIANTS)
...
def gnews_url_en(query: str) -> str:
    df = "&tbs=cdr:1,cd_min:1%2F1%2F2026"
...
def fetch_rss_local(feed_id, base_feed, query, hl, gl, ceid, country):
    try:
        ...
        for entry in parsed.entries[:MAX_ITEMS * 2]:
            ...
            if is_tuvsud(title_local, source):
                continue
            items.append({
