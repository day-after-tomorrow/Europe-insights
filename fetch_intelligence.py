...
LOCAL_KEYWORDS = {
    ...
    "emerging": {
        "pl": "ESG zrównoważony certyfikacja wodór energetyka",
        "cs": "ESG udržitelnost certifikace vodík obnovitelné",
        "sk": "ESG udržateľnosť certifikácia vodík obnoviteľné",
        "hu": "ESG fenntarthatóság tanúsítás hidrogén megújuló",
        "ro": "ESG sustenabilitate certificare hidrogen regenerabil",
        "tr": "ESG sürdürülebilirlik sertifikasyon hidrojen yenilenebilir",
        "fr": "ESG durabilité certification hydrogène renouvelable",
        "it": "ESG sostenibilità certificazione idrogeno rinnovabile",
        "de": "ESG Nachhaltigkeit Zertifizierung Wasserstoff erneuerbar",
        "es": "ESG sostenibilidad certificación hidrógeno renovable",
        "da": "ESG bæredygtighed certificering brint vedvarende",
        "sl": "ESG trajnostnost certificiranje vodik obnovljivi",
    },
}
...
# Direct RSS from national accreditation body websites (primary source).
# Silent failure: if a site has no RSS it returns 0 items, GNews still fills the tile.
ACCRED_RSS = {
...
def gnews_url_en(query: str) -> str:
    return (f"{GNEWS_BASE}?q={quote(query)}"
            f"&hl=en-US&gl=US&ceid=US:en"
            f"&tbs=cdr:1,cd_min:1%2F1%2F2025")
...
# ── Date / relevance / scoring utilities ──────────────────────────────────────
def parse_pub_date(pub: str) -> Optional[datetime]:
    if not pub:
        return None
    try:
    ...
    for fmt in ("%Y-%m-%dT%H:%M:%S%z", "%Y-%m-%d"):
        try:
            return datetime.strptime(pub[:19], fmt).replace(tzinfo=timezone.utc)
        except ValueError:
            pass
    return None
...
def is_tuvsud(title: str, source: str, link: str = "") -> bool:
    """Return True if item is about TÜV SÜD (us) — exclude from competitive feeds.
    Checks title, source, AND link URL (Czech blog posts have tuvsud.com links
    with Czech titles that never mention 'TÜV SÜD' by name)."""
    haystack = (title + " " + source).lower()
...
def score_urgency(title: str) -> str:
    ...
    if any(w in t for w in [
        ...
        "appoints", "hires", "certification", "accreditation",
        "regulation", "compliance", "laboratory", "testing",
        "compliance", "zatrudni", "inwestycj", "przetarg",
        "zaměstn", "investic", "angajare", "investiție", "licitație",
        "işe alım", "yatırım", "ihale", "embauche", "investissement",
        "assunzione", "Einstellung", "contratación",
    ]):
        return "medium"
    return "low"
...
def feed_queries(country: str):
    ...
    # ── 1. Competitor Moves: all 8 named competitors, no industry restriction ──
    # Austria: anchor on Wien/Vienna to prevent Australia matches
    if country == "Austria":
    ...
    # ── 2. M&A: all 8 named competitors + acquisition/merger terms ───────────
    ...
    # ── 3. Market & Regulatory: key TÜV SÜD industries ──────────────────────
    # No year in query — cd_min URL param handles 2025+ cutoff, so both 2025
    # and 2026 articles are returned without suppressing either year.
    ...
    # ── 4. EU Regulatory Watch: EU regulation/directive + TIC ────────────────
    # No year — date filter handles cutoff; hardcoded year suppresses 2026 content
    reg  = "EU regulation directive TIC testing inspection certification"

    # ── 5. Accreditation & Standards ─────────────────────────────────────────
    accred_body = ACCRED.get(country, "")
    std1 = (f"{c} {accred_body} ISO accreditation laboratory standard"
            if accred_body else f"{c} ISO accreditation laboratory standard")
    std2 = (f"{accred_body} accreditation {std_fallback} ISO laboratory"
            if accred_body else f"accreditation {std_fallback} ISO laboratory")

    # ── 6. Investment Projects: nuclear + general industrial ─────────────────
    ...
    # Austria: append "Österreich" to non-competitor queries to suppress AU results
    if country == "Austria":
        base_queries = [
    ...
    # Low-volume markets: add a 3rd broad competitor query for more signal
...
def fetch_accred_rss(country: str):
    # type: (str) -> tuple
    """Fetch news directly from national accreditation body website.
    Returns (feed_id, items). Silent failure — GNews still fills the tile.
    Unlike job boards, accred bodies are public info services with no motive
    to block datacenter IPs. Worst case: 0 items, tile unchanged."""
    ...
    try:
        ...
        if not parsed.entries:
            log.info("  ✓ ACCRED-RSS  (%s/%s) 0 items (feed empty or no RSS)",
                     country, ACCRED.get(country, "?"))
            return ("standards_accred", [])
        items = []
        for entry in parsed.entries[:MAX_ITEMS]:
            ...
            source = (entry.get("source", {}).get("title", "")
                      or src_guess or ACCRED.get(country, "Accred"))
        ...
        log.info("  ✓ ACCRED-RSS  (%s/%s) %d items",
                 country, ACCRED.get(country, "?"), len(items))
        return ("standards_accred", items)
    except Exception as exc:
        # INFO not DEBUG — we want to see which sites have no RSS in the log
        log.info("  ✗ ACCRED-RSS  (%s/%s) %s", country, ACCRED.get(country, "?"), exc)
        return ("standards_accred", [])
...
# ── Feed merging ──────────────────────────────────────────────────────────────
BASE_FEEDS = [
    "competitor","ma","market","regulatory",
...
def fetch_country(country: str) -> dict:
    ...
    # English news queries + direct accreditation body RSS
    ...
    log.info("  → %d items (%d local) for %s",
