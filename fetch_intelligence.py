...
Local-language headlines are translated to English via Google Translate
(free, no API key, no account — same endpoint as translate.google.com).
...
# Google Translate (free, unofficial endpoint — no API key, no account needed)
# Uses the same endpoint as translate.google.com — up to 500k chars/month free.
...
# ── Batch translation via Google Translate (free, no key needed) ──────────────
def translate_one(text: str, src_lang: str) -> str:
    """
    Translate a single string to English using Google Translate's free endpoint.
    Same backend as translate.google.com — no API key, no account, no cost.
    """
    params = {
        "client": "gtx",
        "sl":     src_lang,
...
def translate_batch(items: list[dict], country: str) -> list[dict]:
    """
    Translate all local items (local=True, titleEN=None) to English.
    Uses Google Translate free endpoint — one request per headline,
    run in parallel threads. Falls back to original title on any error.
    """
    ...
    def _translate_item(item):
        try:
            ...
            log.warning("  ✗ Translate '%s…': %s", item["title"][:30], exc)
            item["titleEN"] = item["title"]   # fallback: show original
        return item

    # Translate in parallel — typically 20-40 headlines per country
    with ThreadPoolExecutor(max_workers=8) as pool:
        list(pool.map(_translate_item, to_translate))

    translated_ok = sum(1 for i in to_translate if i.get("titleEN") != i.get("title"))
    log.info("  ✓ Translated %d/%d local headlines for %s (Google Translate, free)",
             translated_ok, len(to_translate), country)
    return items
...
def main():
    ...
    log.info("Translation: Google Translate free endpoint (no API key needed)")
