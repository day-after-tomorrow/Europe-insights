...
CONFIRMED_JOB_BOARDS = {
    ...
    # PL, RO, TR, BE, SI — covered by Google News job queries only
}
...
def fetch_job_rss(board_name: str, url: str, country: str) -> list[dict]:
    """
...
def fetch_job_gnews(company: str, query: str, country: str) -> list[dict]:
    """
...
def fetch_wb_indicator(wb_code: str, indicator: str, label: str) -> dict | None:
    """
    Fetch a single World Bank indicator for one country.
    Returns {value, year, source} or None.
    mrv=5 fetches 5 most-recent years so we always find a non-null value.
    """
    try:
        url = (
        ...
        resp.raise_for_status()
        data = resp.json()
        # data is [{metadata}, [records...]] — records sorted newest-first
        records = data[1] if len(data) > 1 else []
        rows = [r for r in (records or []) if r.get("value") is not None]
        if rows:
            r = rows[0]
            val = round(float(r["value"]), 2)
            log.info("  ✓ WB %-12s %s → %.2f%% (%s)", label, wb_code, val, r["date"])
            return {"value": val, "year": r["date"], "source": "World Bank"}
        log.warning("  ✗ WB %-12s %s — no data", label, wb_code)
        return None
    except Exception as exc:
        log.warning("  ✗ WB %-12s %s: %s", label, wb_code, exc)
        return None

def fetch_macro(country: str) -> dict:
    """
    GDP growth (NY.GDP.MKTP.KD.ZG) and CPI inflation (FP.CPI.TOTL.ZG)
    from World Bank for all 15 countries. Runs both in parallel.
    """
